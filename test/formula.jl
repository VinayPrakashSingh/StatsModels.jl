...
## Formula parsing
import StatsModels: @~, Formula, Terms, AbstractTerm
...
t = Terms(y ~ 1 + x1 + x2)
@test t.intercept == true
@test t.terms == AbstractTerm[:x1, :x2]
...
## `*` expansion
t = Terms(y ~ x1 * x2)
@test t.terms == AbstractTerm[:x1, :x2, :(x1 & x2)]
@test t.eterms == [:y, :x1, :x2]

## associative rule:
## +
t = Terms(y ~ x1 + x2 + x3)
@test t.terms == AbstractTerm[:x1, :x2, :x3]
...
t = Terms(y ~ x1 & (x2 + x3))
@test t.terms == AbstractTerm[:(x1&x2), :(x1&x3)]
...
##
## t = Terms(y ~ (x2 + x3) & x1)
## @test t.terms == AbstractTerm[:(x2&x1), :(x3&x1)]
...
## t = Terms(y ~ 1 & x1)
## @test t.terms == AbstractTerm[:x1]              # == [:(1 & x1)]
## @test t.eterms == [:y, :x1]

## t = Terms(y ~ (1 + x1) & x2)
## @test t.terms == AbstractTerm[:x2, :(x1&x2)]    # == [:(1 & x1)]
## @test t.eterms == [:y, :x1, :x2]
